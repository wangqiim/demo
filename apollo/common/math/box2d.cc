...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const {
  ...
  std::cerr << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " "
            << gy2;
  std::abort();
  return 0.0;
}
...
std::string Box2d::DebugString() const {
  return "box2d ( center = " + center_.DebugString() + "  heading = " + std::to_string(heading_) + "  length = " + std::to_string(length_) + "  width = " + std::to_string(width_) + " )";
}
