...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  if (!is_convex_) {
    Polygon2d convex_polygon;
    ComputeConvexHull(points_, &convex_polygon);
    assert(convex_polygon.is_convex() == true);
  ...
  assert(ComputeConvexHull(points, &new_polygon) == true);
  return new_polygon;
}
...
std::string Polygon2d::DebugString() const {
  ...
  std::string is_convex_str = (is_convex_ ? "convex" : "non-convex");
  return "polygon2d (  num_points = " + std::to_string(num_points_) + "  points = (" + points_str +
                      " )  " + is_convex_str + "  area = " +  std::to_string(area_) + " )";
}
